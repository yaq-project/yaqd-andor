...
class AndorSdk2Ixon(_andor_sdk2.AndorSDK2):
    ...
    def __init__(self, name, config, config_filepath):
        ...
        xai,yai,bin=self._set_aoi()
        self._gen_mappings(xai, yai, bin)
    ...
    def _set_aoi(self):
        ...
        x_ai = np.arange(int(left), int(left + width * binning), int(binning), dtype=int)[None, :]
        y_ai = np.arange(int(top), int(top + height * binning), int(binning), dtype=int)[:, None]

        return x_ai, y_ai, binning
