...
class atmcd:
    ...
    def acquire_series(self, lenght: int) -> List[np.ndarray]:
        series = []
    ...
    def GetTemperatureStatus(self):
        ...
        ret = self.dll.GetTemperatureStatus(
            byref(cSensorTemp), byref(cTargetTemp), byref(cAmbientTemp), byref(cCoolerVolts)
        )
    ...
    def GetUSBDeviceDetails(self):
        ...
        ret = self.dll.GetUSBDeviceDetails(
            byref(cVendorID),
            byref(cProductID),
            byref(cFirmwareVersion),
            byref(cSpecificationNumber),
        )
        return (
            ret,
            cVendorID.value,
            cProductID.value,
            cFirmwareVersion.value,
            cSpecificationNumber.value,
        )
    ...
    def PostProcessNoiseFilter(
        self, pInputImage, iOutputBufferSize, iBaseline, iMode, fThreshold, iHeight, iWidth
    ):
        """
        Description:
    ...
    def SaveAsCalibratedSif(self, path, data_type, unit, coeff, rayleighWave):
        ...
        ret = self.dll.SaveAsCalibratedSif(
            byref(cpath), cdata_type, cunit, byref(ccoeff), crayleighWave
        )
        return ret
    ...
    def SetIsolatedCropModeEx(
        self, active, cropheight, cropwidth, vbin, hbin, cropleft, cropbottom
    ):
        """
        Description:
    ...
    def WhiteBalance(self):
        ...
        ret = self.dll.WhiteBalance(
            byref(cwRed), byref(cwGreen), byref(cwBlue), byref(cfRelR), byref(cfRelB), byref(cinfo)
        )
